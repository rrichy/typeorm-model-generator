...
class EntityFileToJson {
    static getEntityOptions(trimmedLine, ent) {
        ...
        if (decoratorParameters.length > 0) {
            if (decoratorParameters[0] !== '"' ||
                ...
                ent.entityOptions = JSON.parse(removeTrailingComas(badJSON.replace(/(['"])?([a-z0-9A-Z_]+)(['"])?:/g, '"$2": ')));
            }
        }
    }
    static getColumnOptionsAndType(trimmedLine, col) {
        const decoratorParameters = trimmedLine.slice(trimmedLine.indexOf("(") + 1, trimmedLine.lastIndexOf(")"));
        const primaryGeneratedColumn = trimmedLine.substring(0, trimmedLine.indexOf("(")) ===
        ...
        if (decoratorParameters.length > 0) {
            if (decoratorParameters.search(",") > 0 &&
                ...
                col.columnOptions = JSON.parse(removeTrailingComas(badJSON.replace(/(['"])?([a-z0-9A-Z_]+)(['"])?:/g, '"$2": ')));
            }
            else if (decoratorParameters[0] === '"' &&
                decoratorParameters.endsWith('"')) {
                col.columnTypes = decoratorParameters
                    .split("|")
                    .map(x => x.trim());
                ...
                let badJSON = !primaryGeneratedColumn
                    ? decoratorParameters.substring(decoratorParameters.indexOf(",") + 1)
                    : decoratorParameters;
    ...
    static getRelationOptions(trimmedLine, col) {
        const decoratorParameters = trimmedLine.slice(trimmedLine.indexOf("(") + 1, trimmedLine.lastIndexOf(")"));
        if (decoratorParameters.length > 0) {
            const params = decoratorParameters.match(/(,)(?!([^{]*}))/g);
            if (params && params.length === 2) {
                let badJSON = decoratorParameters
                    .substring(decoratorParameters.lastIndexOf("{"), decoratorParameters.lastIndexOf("}") + 1)
                    .trim();
                if (badJSON.lastIndexOf(",") === badJSON.length - 3) {
                    badJSON =
                        badJSON.slice(0, badJSON.length - 3) +
                ...
                col.columnOptions = JSON.parse(removeTrailingComas(badJSON
                    .replace(/(')/g, `"`)
                    .replace(/(['"])?([a-z0-9A-Z_]+)(['"])?:/g, '"$2": ')));
            }
        }
    }
    static getIndexOptions(trimmedLine, ind) {
        const decoratorParameters = trimmedLine.slice(trimmedLine.indexOf("(") + 1, trimmedLine.lastIndexOf(")"));
        if (decoratorParameters.length > 0) {
            ...
            if (containsName) {
                ind.indexName = decoratorParameters.slice(decoratorParameters.indexOf('"') + 1, decoratorParameters
            ...
            if (containsTables) {
                ...
                ind.columnNames.push(...columnsStr
                    .split(",")
                    .map(val => {
                    let colName = "";
                    if (val.search("\\.") > -1) {
        ...
        lines.forEach(line => {
            ...
            if (!isInClassBody) {
                ...
                if (trimmedLine.startsWith("export class")) {
                    retVal.entityName = trimmedLine
                        .substring(trimmedLine.indexOf("class") + 5, trimmedLine.lastIndexOf("{"))
                        .trim()
                        .toLowerCase();
                    isInClassBody = true;
                    return;
            ...
            if (trimmedLine.startsWith("@JoinColumn")) {
                if (EntityFileToJson.isPartOfMultilineStatement(trimmedLine)) {
                    ...
                    const decoratorParameters = trimmedLine
                        .substring(trimmedLine.indexOf("(") + 1, trimmedLine.indexOf(")"))
                        .trim()
                        .replace(/(['"])?([a-z0-9A-Z_]+)(['"])?:/g, '"$2": ');
                    if (decoratorParameters.length > 0) {
                        const column = retVal.columns[retVal.columns.length - 1];
            ...
            if (trimmedLine.split(":").length - 1 > 0) {
                ...
                if (colTypes.startsWith("Promise<")) {
                    ...
                    retVal.columns[retVal.columns.length - 1].columnOptions.isTypeLazy = true;
                }
                retVal.columns[retVal.columns.length - 1].columnTypes = colTypes.split("|").map(x => {
                    if (x === "any") {
                ...
                if (!retVal.columns[retVal.columns.length - 1].columnTypes.some(val => val === "null")) {
                    retVal.columns[retVal.columns.length - 1].columnTypes.push("null");
                }
                if (retVal.indices.length > 0 &&
                    retVal.indices[retVal.indices.length - 1].columnNames
                        .length === 0) {
                    retVal.indices[retVal.indices.length - 1].columnNames.push(retVal.columns[retVal.columns.length - 1].columnName);
                }
                return;
            }
            if (trimmedLine === "}") {
    ...
    static isPartOfMultilineStatement(statement) {
...
exports.default = EntityFileToJson;
