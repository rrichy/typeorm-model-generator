...
const IConnectionOptions_1 = require("../../src/IConnectionOptions");
const runTestsFromPath_test_1 = require("../integration/runTestsFromPath.test");
chai.use(chaiSubset);
const { expect } = chai;
...
describe("Model customization phase", async () => {
    ...
    describe("case-file", () => {
        it("PascalCase", () => {
            ...
            const customizedModel = (0, ModelCustomization_1.default)(data, generationOptions, {});
            (0, ModelGeneration_1.default)((0, IConnectionOptions_1.getDefaultConnectionOptions)(), generationOptions, customizedModel);
            ...
            (0, runTestsFromPath_test_1.compileGeneratedModel)(generationOptions.resultsPath, [""]);
        });
        it("camelCase", () => {
            const data = generateSampleData();
            const generationOptions = generateGenerationOptions();
    ...
    describe("case-entity", () => {
        ...
        it("camelCase", () => {
            ...
            expect(postContent).to.contain("class post {");
            expect(postAuthorContent).to.contain("class postAuthor {");
            (0, runTestsFromPath_test_1.compileGeneratedModel)(generationOptions.resultsPath, [""], false);
        });
    });
    ...
    it("activeRecord", async () => {
        ...
        const postAuthorContent = fs
            .readFileSync(path.resolve(filesGenPath, "PostAuthor.ts"))
            .toString();
        expect(postContent).to.have.string(`export class Post extends BaseEntity `);
        expect(postAuthorContent).to.have.string(`export class PostAuthor extends BaseEntity `);
        (0, runTestsFromPath_test_1.compileGeneratedModel)(generationOptions.resultsPath, [""]);
    });
    it("skipSchema", async () => {
        const data = generateSampleData();
    ...
    it("generateConstructor", async () => {
        ...
        expect(postAuthorContent).to.have.string(`constructor(init?: Partial<PostAuthor>)`);
    ...
    it("naming strategy", async () => {
        ...
        expect(postContent).to.have.string(`import { PostAuthor_B } from "./PostAuthor_B_D";`);
        expect(postAuthorContent).to.have.string(`@Entity("PostAuthor"`);
        expect(postAuthorContent).to.have.string(`class PostAuthor_B`);
        expect(postAuthorContent).to.have.string(`id_C: number;`);
        expect(postAuthorContent).to.have.string(`import { Post_B } from "./Post_B_D";`);
        (0, runTestsFromPath_test_1.compileGeneratedModel)(generationOptions.resultsPath, [""], false);
    });
...
//# sourceMappingURL=modelCustomization.test.js.map